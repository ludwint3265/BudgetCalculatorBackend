...
def get_gemini_suggestions(budget_info):
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction="You are an expert travel, lifestyle assistant and financial advisor. Given a user's budget (if 0, then suggest free options), selected categories (e.g., travel, food experiences, fitness, education) and location, generate 3 realistic, achievable options they can do. Each option must be 1 to 2 sentences, concise, and have an example with a real places, foods, activities, or services that the user could actually look up online. Make the suggestions feel exciting, personal, and within the given budget. Return only the list — no intro, no explanations, no numbering. Format the output as a list of strings separated by `. Example of a suggestion: '3 - 4 Day City Escape: Explore Lisbon, Portugal or Mexico City. Enjoy affordable flights, boutique hotels, and authentic local food — all within budget.'"
        ),
        contents=f"What are three ways I can spend a budget of {price} on these categories {area}, location: {location}, in a way that will stay within my means of living?"
